...
shadow-workipedia/
├── public/
│   ├── graph.json             # Generated - DO NOT EDIT MANUALLY (nodes/edges; blocks first paint)
│   ├── articles.json          # Generated - DO NOT EDIT MANUALLY (wiki prose; loaded lazily)
...
- `public/graph.json` and `public/articles.json` are generated by `pnpm extract-data`
- Edit the source files in the parent repo instead, then re-extract

### 2. Data Pipeline
```bash
# After changes to parent repo catalog/connections:
pnpm extract-data    # Regenerate graph.json + articles.json from parent repo
...
### Public Data (`public/`)
- `graph.json` / `articles.json` - Generated graph data and wiki prose (DO NOT EDIT)
...
2. Run `pnpm extract-data` to update graph.json + articles.json
3. Verify in browser

### Debug Graph Issues
- Check browser console for D3/canvas errors
- Verify `graph.json` has correct node/edge structure
