...
## Provenance — what the first pass got wrong, and what production still serves
...
**The "resolved" column describes the repository, not the deployed site.** Verified 2026-09-20:
the Cloudflare Pages project `shadow-workipedia` has **no git integration** (`source: null`), so
pushing to `main` has never deployed anything. Production is still serving deployment `43f0c52c`,
built from commit **`632e730`** on **2026-07-14**, whose `data.json` was generated **2026-01-07**.
That is the same commit the first pass measured — so the "Stale HEAD" column below is an accurate
description of what visitors get today, and the defects marked resolved are resolved only upstream.
Live right now: 1,048 issue-system edges, 0 issues carrying `tags`, 14 of 22 systems with no
`connectionCount`. Closing that gap is a deploy, not a code change — see `shadow-work-mt0n`.

| Claim | Stale HEAD = **live production** | origin/main |
|---|---|---|
| Descriptions that are crisis headlines | 222/353 | **0** — resolved in repo only |
| Systems with `connectionCount: 0` | 14/22 | **0** — resolved in repo only |
| Urgency mismatches vs source YAML | 156/353 | **0** — resolved in repo only |
